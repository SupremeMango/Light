    ...
    try {
        ...
        const htmlContent = novels.map((novel, index) => {
            ...
            if (match && match[1]) {
                ...
                // The feature I thought was probably the one that searches for the name and stuff, although I don't know what to do. 
                // UMM, we might have a lead.
            }
        ...
        const synced_data = await auto_sync();
        if (synced_data.length == 0) {
            console.warn("⚠️ Cookie Expired or empty response.");
            return;
        }
        else {
            // console.log("Yay! Cooking is working!", synced_data);
            results = await filter_hash(synced_data, novel_list);
            
            // 6, if there is an update, then, we update these links.
            requestAnimationFrame(() => {
...
// Auto_sync() is a function that gets the data from sync_novels()
async function auto_sync() {
    try {
        ...
        console.error("Failed to fetch novels: Or cookie failed?", error);
    }
}
...
function local_update(item) {
    ...
    if (div) {
        ...
        alert("We are done syncing sir")
        // We have to wait until some are done syncing.
...
// Get the results from auto_sync()


// These temporary data should be kept to develop other features later.
// const synced_data = [
//     {
